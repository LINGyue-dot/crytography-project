package cn.qianlon.crytography.utils.MD5;
...
public class Md5 {
    ...
    //4轮循环中使用的生成函数(轮函数)g
    private static long g(int count, long b, long c, long d) {
        switch (count) {
            case 0://函数F
                return (b & c) | ((~b) & d);
            case 1://函数G
    ...
    //MD5加密
    private String exchange(String message) {
        //转化为byte
        byte[] inputBytes = message.getBytes();
        //获取字节数组的长度
        int length = inputBytes.length;
        //得到K值（以bit作单位的message长度）
        long K = (long) (length * 8);
        //完整小组(512bit)(64byte)的个数
        int groupCount = length / 64;
        ...
        for (int i = 0; i < rest; i++)
            paddingBytes[i] = inputBytes[length - rest + i];
    ...
    public static String decryMd5(String originStr){
        Md5 md = new Md5();
        return md.exchange(originStr).toUpperCase();
    }
...
//        System.out.println("Result Message: " + md.exchange(message));
